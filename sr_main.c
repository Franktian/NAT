...
int main(int argc, char **argv)
{
    ...
    sr.enable_nat = 0;
    struct sr_nat nat;
    nat.icmp_query_timeout = DEFAULT_ICMP_QUERY_TIMEOUT;
    nat.tcp_established_timeout = DEFAULT_TCP_ESTABLISHED_IDLE_TIMEOUT;
    nat.tcp_transitory_timeout = DEFAULT_TCP_TRANSITORY_IDLE_TIMEOUT;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:I:E:R:n")) != EOF)
    {
        switch (c)
        {
            ...
            case 'T':
                ...
                break;
            case 'I':
                nat.icmp_query_timeout = atoi((char *) optarg);
                break;
            case 'E':
                nat.tcp_established_timeout = atoi((char *) optarg);
                break;
            case 'R':
                nat.tcp_transitory_timeout = atoi((char *) optarg);
                break;
            case 'n':
