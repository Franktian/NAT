...
uint16_t tcp_cksum(const void *_data, int len) {
  const uint8_t *packet = _data;

  sr_ip_hdr_t *ip_hdr = (sr_ip_hdr_t *)(packet + sizeof(sr_ethernet_hdr_t));
  sr_tcp_hdr_t *tcp_hdr = (sr_tcp_hdr_t *)(packet + sizeof(sr_ethernet_hdr_t) +
                                                    sizeof(sr_ip_hdr_t));
  ...
  pseudo_hdr->tcp_len = htons(sizeof(sr_tcp_hdr_t) + data_size);

  tcp_hdr->tcp_sum = 0;

  unsigned int total_len = sizeof(sr_tcp_pseudo_hdr_t) + sizeof(sr_tcp_hdr_t) + data_size;

  unsigned char *buf = malloc(total_len);
  memcpy(buf, pseudo_hdr, sizeof(sr_tcp_pseudo_hdr_t));
  memcpy(buf + sizeof(sr_tcp_pseudo_hdr_t), tcp_hdr, sizeof(sr_tcp_hdr_t) + data_size);

  uint16_t tcp_sum = cksum(buf, total_len);
...
void print_hdr_icmp_echo(uint8_t *buf) {
  sr_icmp_echo_hdr_t *icmp_hdr = (sr_icmp_echo_hdr_t *)(buf);
...
void print_hdr_tcp(uint8_t *buf) {
  ...
  fprintf(stderr, "\tsource port: %d\n", tcp_hdr->tcp_src_port);
  fprintf(stderr, "\tdestination port: %d\n", tcp_hdr->tcp_dst_port);
  fprintf(stderr, "\tsequence: %d\n", tcp_hdr->tcp_seq_num);
  fprintf(stderr, "\tacknowledge number: %d\n", tcp_hdr->tcp_ack_num);
  fprintf(stderr, "\tchecksum: %d\n", tcp_hdr->tcp_sum);
}
